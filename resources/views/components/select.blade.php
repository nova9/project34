...
@php
    ...
    // Find the selected item by value, or set to null if not found
    $selectedItem = $selected !== null
        ? collect($items)->firstWhere('value', $selected)
        : null;
    $selectedJson = $selectedItem ? json_encode($selectedItem) : 'null';
...
<div
    ...
    @if($name)
        <input type="hidden" name="{{ $name }}" x-model="selectedItem.value">
    @endif
